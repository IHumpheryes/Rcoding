#--- ####################################################################
#--- title: "RICT IV CLASSIFICATION FUNCTIONS version 2"                #
...
########################################################################################################################################
# This R Script is a functional module that aims to define "all" classification functions in all models - GB, NI                       #
# It replaces both ClassificationfunctionsV2.R by editing out and commenting on usag with both GB and NI models                        #
# All RICT IV models, GB, NI etc can use this script for all the routine classification processes ,all in one place                    #
# Works with Predictionfunctions v2 and Helperfucntions v1                                                                             #
######################################################################################################################################## 
...
getClassarray_ntaxa_old <- function (EQR_ntaxa) {
...
getClassarray_aspt_old <- function (EQR_aspt) {
  #classArray_siteOne <- data.frame(nrow= nrow(EQR_ntaxa_spr))
  # Classify these for SITE ONE using the EQR just for spring
...
getAvgEQR_SprAut <- function (EQR_spr,EQR_aut,k ) {
    ...
    #print(eqr_av)
    row.names(eqr_av) <- c(paste0("TST-",k))
    return (eqr_av)
}
...
# 18 REading big files with fread. this function is duplicated in Helperfunctions.R
